...
class I18n {
    ...
    translations = {
        fr: {
            ...
            languageDetection: {
                detected: 'Langue détectée: français. Vous pouvez la changer avec les drapeaux en haut à droite.',
                icon: '🌍'
            }
        },
        ...
        en: {
            ...
            // Messages de détection automatique
            languageDetection: {
                detected: 'Language detected: English. You can change it using the flags in the top right.',
                icon: '🌍'
            }
        }
    };
    ...
    detectBrowserLanguage() {
        ...
        if (this.translations[langCode]) {
            console.log(`Langue du navigateur détectée: ${browserLang} -> ${langCode}`);
            return langCode;
        }
    ...
    async loadLanguageFromStorage() {
        try {
            const result = await chrome.storage.local.get(['language', 'languageAutoDetected', 'languageManuallySet']);
            ...
            if (result.language && result.languageManuallySet) {
                ...
                this.currentLanguage = result.language;
                console.log(`Langue chargée (auto-détectée précédemment): ${this.currentLanguage}`);
            } else {
                ...
                console.log(`Première utilisation: langue auto-détectée et sauvegardée: ${detectedLang}`);
    ...
    async setLanguage(lang) {
        if (this.translations[lang]) {
            ...
            const notification = document.querySelector('.language-detection-notification');
            ...
            try {
                ...
                if (tasks.length > 0) {
                    const updatedTasks = tasks.map(task => {
                        ...
                        return {
                            ...task,
                            description: this.getFrequencyDescription(intervalValue, timeUnit)
                        };
                    });
    ...
    getFrequencyDescription(value, unit) {
        ...
        const key = isPlural ? `frequencyDescription.plural.${unit}` : `frequencyDescription.single.${unit}`;
    ...
    async resetLanguageDetection() {
        try {
            await chrome.storage.local.remove(['language', 'languageAutoDetected', 'languageManuallySet']);
