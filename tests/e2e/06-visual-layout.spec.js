...
test.describe('AutoAgent - Tests visuels et layout', () => {
  ...
  test('should have consistent visual styling across all modes', async ({ page }) => {
    const modes = [
      { selector: '[data-unit="hours"]', config: '#hoursConfig', name: 'heures' },
      { selector: '[data-unit="days"]', config: '#daysConfig', name: 'jours' },
      { selector: '[data-unit="weeks"]', config: '#weeksConfig', name: 'semaines' }
    ];
  ...
  test('should maintain proper spacing and alignment', async ({ page }) => {
    ...
    if (containerBox) {
      expect(containerBox.width).toBeGreaterThan(500); // Extension doit être assez large
    }
    
    // Vérifier l'alignement des sections principales
    const header = page.locator('.header');
    const createSection = page.locator('.create-section');
    const taskSection = page.locator('.task-section-header');
  ...
  test('should validate schedule container positioning', async ({ page }) => {
    ...
    for (const mode of modes) {
      ...
      if (containerBox && configBox) {
        ...
        expect(configBox.x + configBox.width).toBeLessThanOrEqual(containerBox.x + containerBox.width + 5); // Tolérance de 5px
