...
test.describe('AutoAgent - Validation des optimisations', () => {
  ...
  test('should keep all configurations inside the bordered container', async ({ page }) => {
  ...
  test('should maintain perfect vertical alignment across modes', async ({ page }) => {
  ...
  test('should have compact layout with optimized space usage', async ({ page }) => {
  ...
  test('should validate the fixed container height and positioning', async ({ page }) => {
    ...
    for (const mode of ['hours', 'days', 'weeks']) {
      ...
      expect(configBox.y + configBox.height).toBeLessThanOrEqual(containerBox.y + containerBox.height + 5);
    }
    
    console.log('✅ Positionnement dans le conteneur validé');
  });

  test('should capture final state for visual regression testing', async ({ page }) => {
    ...
    for (const mode of ['hours', 'days', 'weeks']) {
      ...
      await page.locator('#promptText').fill(`Prompt de test pour le mode ${mode}`);
