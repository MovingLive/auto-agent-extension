...
test.describe('AutoAgent - Système de planification', () => {
  ...
  test('should display frequency selector with three modes', async ({ page }) => {
  ...
  test('should maintain configurations inside the bordered container', async ({ page }) => {
    ...
    const modes = [
      ...
      { button: '[data-unit="weeks"]', config: '#weeksConfig', name: 'semaines' }
    ];
    ...
    for (const mode of modes) {
      ...
      if (configBox && containerBox) {
        ...
        expect(configBox.y + configBox.height).toBeLessThanOrEqual(containerBox.y + containerBox.height);
      }
